...
describe("SendGridProvider", () => {
  ...
  test("should throw error if SENDGRID_API_KEY is missing", () => {
    ...
    expect(() => new SendGridProvider()).toThrow("Missing SendGrid API key is missing.");
  });

  test("should throw error if SENDGRID_FROM is missing", () => {
    delete process.env.SENDGRID_FROM;
    expect(() => new SendGridProvider()).toThrow("SendGrid sender email address is missing.");
  });

  test("should send an email successfully", async () => {
    (sgMail.send as jest.Mock).mockResolvedValueOnce([{}, {}]);

    await sendGridProvider.sendEmail("recipient@example.com", "Test Subject", "Test Body");
  ...
  test("should throw an error when email sending fails", async () => {
    ...
    await expect(
      sendGridProvider.sendEmail("recipient@example.com", "Test Subject", "Test Body")
